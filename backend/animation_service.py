...
def resolve_visual_plan(
    ...
    *,
    ...
    Prefer `python -m backend.build_once` / POST /build for full semantic bundles.
    """
    ...
    # Legacy path still returns a visual plan only (no semanticSpec).
    ...
    validate_visual_plan(
        ...
        problem,
        execution,
        observed_cell_values=set(observed.cellValues),
    )
...
def create_animation_package(
    ...
    return AnimationPackage(
        ...
        semanticSpec=load_semantic_spec(problem_id),
    )
...
def get_client_package(
    ...
    *,
    ...
    """Page-load package: facts + saved visual plan + semantic + initial state."""
    ...
    if force_visual:
        ...
        semantic = load_semantic_spec(problem_id)
