...
def analyze(
    ...
    structure_type: SceneType = "graph",
) -> ObservedTraceSchema:
        ...
        if isinstance(nodes, list):
            for node in nodes:
                if isinstance(node, dict):
                    collect_scalar_values(node.get("label"), cell_values)
                    collect_scalar_values(node.get("id"), cell_values)
        collect_scalar_values(event.get("value"), cell_values)
        collect_scalar_values(event.get("indegree"), cell_values)
